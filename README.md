...
Clone or download the code from this repository, which includes the necessary files and `start.bat` script for setup and execution.

#### Step 2. Execute the interactive CLI

To start generating keys and the `deposit_data-*.json` file, run the `start.bat` file. This will launch the interactive CLI, where you can choose to create a new mnemonic or use an existing one.

Simply double-click `start.bat` or execute it from the command line:
